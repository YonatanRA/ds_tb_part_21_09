...
x <- c(10,20,NA,30,NA,40) # Vector con valores NA (Not available)
is.na(x) # Esta función permite conocer los valores NA
sum(is.na(x))
x[!is.na(x)] # Excluyo valores NA
mean(x, na.rm=TRUE) # Muchas funciones permiten ignorar los NAs
...
hist(df$Ozone, col="antiquewhite3",main="Histogram of Ozone",
     xlab="Observations",breaks=15)

# Valores fuera del rango inter cuartílico
ric <- IQR(df$Ozone)
q2 <- quantile(df$Ozone,0.75)
sum(df$Ozone > q2+1.5*ric)
df[df$Ozone > q2+1.5*ric,]
